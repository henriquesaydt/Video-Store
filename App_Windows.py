...
class LoginScreen:
    ...
    def Login(self):
        ...
        if c.fetchone() is not None:
            ...
            self.txtLoginError["text"] = "Usuário ou Senha Inválidos!"
...
class MainScreen:
        ...
        if DVDSelecionado and ClienteSelecionado:
            ...
            for i in resul:
                ...
                if int(DVDID) in dvd_codes:
                    self.txtStatus['text'] = "Esse DVD já está alugado!"
                    return
            ...
            self.txtStatus['text'] = "ERRO: Você deve selecionar um cliente e um DVD!"
        ...
        if DVDSelecionado and ClienteSelecionado:
            ...
            for i in resul:
                ...
                if not(int(DVDID) in dvd_codes) and (i['id'] == int(ClienteID)):
                    self.txtStatus['text'] = "Esse DVD não foi alugado por esse cliente"
                    return
    ...
    def ExcluirCliente(self):
        ...
        if ClienteSelecionado:
            ...
            self.txtStatus['text'] = "ERRO: Você deve selecionar um cliente"
    ...
    def ExcluirDVD(self):
        ...
        if DVDSelecionado:
            ...
            self.txtStatus['text'] = "ERRO: Você deve selecionar um DVD"
...
class ClienteScreen:
    ...
    def criarCliente(self):
        ...
        if resul.fetchone():
            self.txtStatus['text'] = "ERRO: Já existe um cliente cadastrado com esse CPF!"
            return
...
class FuncionarioScreen:
    ...
    def criarFuncionario(self):
        ...
        if not FuncionarioUsuario or not FuncionarSenha:
            self.txtStatus['text'] = "ERRO: Insira um usuário e uma senha!"
            return

        resul = c.execute("SELECT * FROM Funcionarios WHERE username=?", (FuncionarioUsuario,))
        if resul.fetchone():
            self.txtStatus['text'] = "ERRO: Já existe um funcionario cadastrado com esse nome de Usuário!"
            return
    ...
    def excluirFuncionario(self):
        ...
        if resul:
            ...
            self.txtStatus['text'] = "ERRO: Esse Funcionario não existe!"
            return
...
class DVDScreen:
    def __init__(self, master=None):
        ...
        self.txtCodDVD = Label(self.frmCodDVD, bg=self.bgColor, fg=self.fgColor, text="Código do DVD:", font=self.fonte12)
    ...
    def criarFilme(self):
        ...
        if not DVDFilme or not DVDCod_dvd:
            self.txtStatus['text'] = "ERRO: Insira um Filme e o código do DVD!"
            return

        resul = c.execute("SELECT * FROM DVD WHERE cod_dvd=?", (DVDCod_dvd,))
        if resul.fetchone():
            self.txtStatus['text'] = "ERRO: Já existe um DVD cadastrado com esse Código de DVD!"
            return
